...
/**
 * fetch price of Bitcoin in time
 * @param currentTimeFrame Time interval (e.g., 1m, 5m, 30m)
 * @returns data price bitcoin
 */

export const GetPriceBitcoin = async (
    currentTimeFrame: string
): Promise<ICandleStick[]> => {
    try {
        const response = await axios.get<
            [number, string, string, string, string, string, number, string, number, string, string, string][]
        >(
            `https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=${currentTimeFrame}&limit=2`
        );
